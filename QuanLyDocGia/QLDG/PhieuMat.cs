...
namespace QLDG
{
    public partial class PhieuMat : Form
    {
        public PhieuMat()
        {
            InitializeComponent();
        ...
        private void PhieuMat_Load(object sender, EventArgs e)
        {
            ...
            if (File.Exists("DuongdanMat.txt"))
            {
                //mở file để đọc
                StreamReader sr = new StreamReader("DuongdanMat.txt");
                //đọc từng dọc
                xuatMat_duongdan.Text = $@"{sr.ReadLine()}";
                sr.Close();
            }
            else xuatMat_duongdan.Text = $@"C:\Users\";
            xuatMat_ten.Text = $"L_{dg.HoTen}_{NgayGhiNhan.ToString("dd-MM-yyyy")}";
        }

        private void xuatMat_Thoat_Click(object sender, EventArgs e)
        {
            this.Close();
        }

        private void buttonxuatMat_duongDan_Click(object sender, EventArgs e)
        {
            using (var fbd = new FolderBrowserDialog())
            {
                ...
                {
                    xuatMat_duongdan.Text = fbd.SelectedPath;
                    xuatMat_duongdan.Text += "\\";
                }
                FileStream fs1 = new FileStream(@"DuongdanMat.txt", FileMode.Create, FileAccess.Write, FileShare.ReadWrite);
                StreamWriter sw = new StreamWriter(fs1);
                sw.WriteLine(xuatMat_duongdan.Text);
                sw.Close();
            }
        }
